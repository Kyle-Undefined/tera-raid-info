...
export class TypeCalc {
    weakness(type: string): Result {
        ...
        Object.entries(defense).forEach(([key, value]) => {
            switch(key) {
                case('double'):
                    value.forEach((i: string) => { results[i] ? results[i] *= 2 : results[i] = 2 });
                    break;
                case('half'):
                    value.forEach((i: string) => { results[i] ? results[i] *= 0.5 : results[i] = 0.5 });
                    break;
                case('zero'):
                    value.forEach((i: string) => { results[i] = 0 });
                    break;
            }
        });
