...
const pokemonImageNormal = document.getElementById('pokemonImageNormal') as HTMLDivElement;
const pokemonImageShiny = document.getElementById('pokemonImageShiny') as HTMLDivElement;
const pokemonAbility = document.getElementById('pokemonAbility') as HTMLDivElement;
const pokemonStatsWrapper = document.getElementById('pokemonStatsWrapper') as HTMLDivElement;
...
const pokemonTypeAdvantages = document.getElementById('pokemonTypeAdvantages') as HTMLDivElement;
const pokemonTeraWeaknesses = document.getElementById('pokemonTeraWeaknesses') as HTMLDivElement;
const pokemonTeraAdvantages = document.getElementById('pokemonTeraAdvantages') as HTMLDivElement;
...
export class Html {
    clearDiv(section?: string): void {
        const sections = [ 'pokemonTypes', 'pokemonImageNormal', 'pokemonImageShiny', 'pokemonAbility', 'pokemonStatsWrapper', 'pokemonMoves', 'pokemonHerbs', 'pokemonTypeAdvantages', 'pokemonTeraWeaknesses', 'pokemonTeraAdvantages' ];
    ...
    createAbilityDisplay(ability: AbilityDex, hidden?: boolean): string {
        return `<div class="typeMatchupText" data-info="${ ability.desc }">${ ability.name }${ (hidden ? ' (H)' : '') }</div>`;
    }

    createHerbDisplay(herb: HerbDex): string {
        return `<div class="herbPill ${ herb.name.toLowerCase() }">${ herb.name } - ${ herb.chance }%</div>`;
    }
    ...
    createMoveDisplay(move: MoveDex): string {
        let moveDisplay = `<div class="typeMatchupText ${ data.getTypeDexEntry(move.type).name.toLowerCase() }">${ move.name }`;
        ...
        if(move.category != Category.Status) {
            ...
            Object.entries(advantages).sort((a, b) => b[1] - a[1]).forEach(([key]) => {
                display.push(`${ common.capitalize(key) }`);
            });

            if(display) {
                moveDisplay += `<div class="adv">Advantages: ${ display.join(', ') }</div>`;
            }
        }
    ...
    createTypeMatchupDisplay(type: string, matchup: number): string {
        return `<div class="typeMatchupText ${type}">${common.capitalize(type)} - ${matchup}x</div>`;
    }
    ...
    populatePokemonList(): void {
        if(raidTier.value) {
            Object.entries(data.dataSource.pokemon).sort().forEach((pokemon) => {
                const [mon] = pokemon;
    ...
    sortByCategory(a: number, b: number): number {
        if(data.getMoveDexEntry(a).category == Category.Status || data.getMoveDexEntry(b).category == Category.Status) {
            return 1;
        } else {
            return -1;
        }
    ...
    viewPokemonAbilities(): void {
        ...
        data.getPokemonAbilities().forEach(ability => {
            this.updateDiv(pokemonAbility, this.createAbilityDisplay(data.getAbilityDexEntry(ability)));
        });

        if(data.getPokemonHiddenAbility()) {
            this.updateDiv(pokemonAbility, this.createAbilityDisplay(data.getAbilityDexEntry(data.getPokemonHiddenAbility()), true));
        }
    }
    ...
    viewPokemonHerbs(): void {
        ...
        data.getPokemonHerbs().forEach(herb => {
            this.updateDiv(pokemonHerbs, this.createHerbDisplay(data.getHerbDexEntry(herb)));
        });
    }
    ...
    viewPokemonMoves(): void {
        ...
        let types: string[] = [];
        data.getPokemonMoves().sort(this.sortByCategory).forEach(moveNumber => {
        ...
        if(display) {
            this.updateDiv(pokemonTypeAdvantages, '<h3>Type Advantages:</h3>' + display.join(''));
        }
    ...
    viewPokemonStats(): void {
        this.updateDiv(pokemonStatsWrapper, this.createStatsDisplay(data.getPokemonStats()));
    }
    ...
    viewTeraTypeAdvantages(type: string): void {
        ...
        if(display) {
            this.updateDiv(pokemonTeraAdvantages, '<h3>Tera Advantages:</h3>' + display.join(''));
        }
    }
    ...
    viewTypeWeaknesses(type: string): void {
        ...
        if(display) {
            this.updateDiv(pokemonTeraWeaknesses, '<h3>Tera Weaknesses:</h3>' + display.join(''));
        }
    }
}